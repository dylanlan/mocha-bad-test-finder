...
export const isDirectory = (filePath: string) => fs.existsSync(filePath) && fs.lstatSync(filePath).isDirectory();
...
export const getTestFiles = (parentDirectory: string) => {
    ...
    const directories = files.filter((f: string) => isDirectory(path.join(parentDirectory, f))
